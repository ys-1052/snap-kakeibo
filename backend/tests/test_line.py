...
def test_link_line_account_mock_success(mock_dynamodb):
    ...
    with patch("backend.main.os.environ") as mock_env:
        ...
        mock_env.get.side_effect = lambda key, default=None: "true" if key == "MOCK_AUTH_ENABLED" else default
...
def test_unlink_line_account(mock_dynamodb):
    ...
    assert response.status_code == 200
    assert response.json() == {"status": "success", "message": "LINE account unlinked successfully"}
...
def test_line_webhook_unlinked(mock_reply, mock_sig, mock_dynamodb):
    ...
    response = client.post("/api/line/webhook", json=payload, headers={"X-Line-Signature": "dummy"})
    assert response.status_code == 200
...
def test_line_webhook_linked_local_async(mock_process, mock_reply, mock_sig, mock_dynamodb):
