...
def analyze_receipt(
    ...
    try:
        ...
        prompt = """
            ...
            10. 値引きや割引が適用された商品がある場合は、値引き・割引後の実支払価格を price に反映し、品目名(items.name)の末尾に「（値引き）」を付与してください（例：商品名が「牛乳」で、その下に「割引 -20」等の値引き表示があれば、items.nameは「牛乳（値引き）」、priceは割引後の価格にする）。判断できない場合はレシート上の商品行の金額を使用してください。
            ...
            16. 出力するすべての文字列フィールド（店舗名を除く）に含まれるスペースは必ず全角スペース（　）を使用してください。半角スペースは使わないでください。
            ...
            18. 品目名(items.name)は、レシートに記載されている文字列をそのまま（略称や表記揺れ、記号なども含め）抽出してください。一般的な名前への変換や、勝手な省略、翻訳などは行わないでください。（例：レシート上の表記が「コカコーラ５００」であれば、そのまま「コカコーラ５００」と出力し、「コカ・コーラ」や「炭酸飲料」などに書き換えないでください）。ただし、ルール10に基づく値引き商品への「（値引き）」の末尾付与は例外として、必ず行ってください。
