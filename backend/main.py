...
class ReceiptAnalysisResult(BaseModel):
    ...
    category_name: str = Field(description="カテゴリ名。'食費', '日用品', '交際費', '交通費', 'エンタメ', 'その他' のいずれか")
    items: List[ReceiptItem] = Field(default_factory=list, description="購入品目のリスト")
    ...
    try:
        presigned_url = s3_client.generate_presigned_url(
            ...
            Params={
                'Bucket': BUCKET_NAME,
                'Key': file_key,
                'ContentType': f'image/{file_extension[1:] if file_extension != ".jpg" else "jpeg"}'
            },
            ExpiresIn=300 # 5分間有効
        )
...
def analyze_receipt(payload: AnalyzeRequest):
    """
    S3にアップロードされた画像を Amazon Bedrock (Nova Lite) で解析し、JSONに構造化して返却します。
    """
    # 1. S3から画像データを取得
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Uploaded image not found in S3: {str(e)}")
    ...
    try:
        ...
        # 解析結果のテキストを取得
        text_output = bedrock_response['output']['message']['content'][0]['text'].strip()
        ...
        raise HTTPException(status_code=500, detail=f"Bedrock AI processing failed: {str(e)}")
...
def save_transaction(payload: TransactionSaveRequest, user_id: str = "USER#default"):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to save transaction to DynamoDB: {str(e)}")
...
def list_transactions(user_id: str = "USER#default"):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to query DynamoDB: {str(e)}")
