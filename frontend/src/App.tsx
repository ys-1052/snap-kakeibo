...
const INITIAL_TRANSACTIONS: Transaction[] = [
  ...
  },
  {
    ...
    category_name: "交通費",
    items: [
      { name: "Suica チャージ", price: 2000, qty: 1 }
    ],
    memo: "移動用",
...
export default function App() {
  ...
  const [geminiApiKey, setGeminiApiKey] = useState(() => localStorage.getItem('snap_kakeibo_gemini_key') || '');
  ...
  const triggerScan = async () => {
    ...
    if (apiUrl) {
      try {
        ...
        const urlRes = await fetch(`${apiUrl}/api/receipts/presigned-url?filename=${encodeURIComponent(nameParam)}`);
    ...
    <div className="app-container">
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div>
            <h1 style={{ fontSize: '18px', fontWeight: 800, letterSpacing: '-0.5px' }}>
              Snap<span style={{ background: 'var(--grad-accent)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Kakeibo</span>
            </h1>
            <p style={{ fontSize: '10px', color: 'var(--text-secondary)' }}>AI Receipt Scan & Analytics</p>
          </div>
        </div>
        ...
        <div style={{
          ...
          <span style={{ fontSize: '12px', fontWeight: 600, color: '#ffd43b' }}>Gemini 2.5 Active</span>
        </div>
      </header>
        ...
        {activeTab === 'dashboard' && (
          <div>
            ...
            <div className="glass-card" style={{
              background: 'linear-gradient(135deg, rgba(27, 20, 52, 0.9) 0%, rgba(15, 18, 36, 0.9) 100%)',
              position: 'relative',
              overflow: 'hidden'
            }}>
              <div style={{
              ...
              <p style={{ fontSize: '14px', color: 'var(--text-secondary)', fontWeight: 500 }}>今月の総支出</p>
              <h2 className="numeric" style={{ fontSize: '42px', fontWeight: 800, margin: '8px 0', letterSpacing: '-1px' }}>
                ¥{totalMonthlySpend.toLocaleString()}
              </h2>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: '#51cf66' }}>
                <TrendingUp size={16} />
            ...
            <div className="glass-card">
              ...
              <div style={{ width: '100%', height: 160 }}>
                <ResponsiveContainer>
                  <AreaChart data={getLineChartData()} margin={{ top: 5, right: 5, left: -25, bottom: 0 }}>
                    <defs>
                    ...
                    <Tooltip 
                      contentStyle={{ background: 'rgba(18,20,32,0.95)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: '#fff' }} 
            ...
            <div className="glass-card">
              ...
              {getPieChartData().length === 0 ? (
                <p style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '20px 0' }}>データがありません</p>
              ) : (
                      ...
                      <PieChart>
                        ...
                        >
                          {getPieChartData().map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={CATEGORY_COLORS[entry.name] || '#868e96'} />
                          ))}
                        </Pie>
                      </PieChart>
                    </ResponsiveContainer>
                    ...
                    {getPieChartData().map((entry, idx) => (
                      <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '12px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: CATEGORY_COLORS[entry.name] || '#868e96' }} />
            ...
            {!previewUrl && !editData && (
              <div className="glass-card" style={{ padding: '40px 20px', textAlign: 'center' }}>
                <label className="upload-zone" style={{ display: 'block' }}>
                  ...
                  <h3 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '8px' }}>レシート写真をアップロード</h3>
            ...
            {editData && (
              <div className="glass-card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                  ...
                  <button 
                    onClick={() => { setEditData(null); setSelectedFile(null); setPreviewUrl(null); }}
                ...
                <div className="form-group" style={{ marginTop: '20px' }}>
                  ...
                  <input 
                    ...
                    onChange={e => setEditData({ ...editData, total_amount: parseInt(e.target.value) || 0 })}
                  />
                </div>
        ...
        {activeTab === 'history' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>全 {transactions.length} 件</span>
            </div>

            {transactions.map(t => (
              <div className="glass-card" key={t.id} style={{ padding: '16px 20px', marginBottom: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                  <div>
                    <h4 style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-primary)' }}>{t.shop_name}</h4>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px' }}>
                      ...
                      <span className="numeric" style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Calendar size={12} />
                        {t.transaction_date}
                      </span>
                    </div>
                  </div>
                  <span className="numeric" style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-primary)' }}>
                ...
                {t.items && t.items.length > 0 && (
                  <div style={{ 
                    ...
                    {t.items.slice(0, 2).map((item, index) => (
                      <div key={index} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: 'var(--text-secondary)' }}>
                        <span>• {item.name}</span>
        ...
        {activeTab === 'settings' && (
          <div className="glass-card">
            ...
            <div style={{ 
              ...
              <div>
                <h4 style={{ fontSize: '13px', fontWeight: 600, color: '#ec4899', marginBottom: '2px' }}>デモモード対応</h4>
                <p style={{ fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.4 }}>
                  AWS Lambda API URL が未設定の場合、自動的にシミュレータによる「ダミースキャン（デモ）」が実行されます。
            ...
            <div className="form-group">
              ...
              />
              <span style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '4px', display: 'block' }}>
                ※ バックエンド（AWS Lambda）の環境変数 `GEMINI_API_KEY` に設定してデプロイすることを推奨します。
              </span>
            </div>
